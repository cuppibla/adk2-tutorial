...
>
> 1. **On 2.3.0 — this codelab's pinned version — `task` mode cannot be a static graph node.** `Workflow(...)` raises at construction; the scheduler overwrites a node's input with the latest user message on re-entry, which would lose the task brief. ADK's error names the two ways around it: *"use a chat coordinator with task sub-agents"* (what this level does), *"or dispatch dynamically via `ctx.run_node` from a function node."* **This was lifted in 2.5.0** — the restriction the ADK docs describe as "expected to be re-enabled in a future release" has since shipped, and on 2.5.0 a `task` agent works as a graph node directly. If you're on 2.5.0+, ignore this limitation.
> 2. **A `task` agent must be a leaf** — the docs list this under *Known limitations*, but it is a contract, not a runtime guard: neither 2.3.0 nor 2.5.0 will stop you from giving a task agent subagents. Don't read the absence of an error as permission.
