...
> aside negative
> **`403 Permission 'aiplatform.endpoints.predict' denied ... (or it may not exist)`?** This is the single most likely thing to go wrong, and it is **not** a billing, credit, or permission problem — the lines above already confirmed your project exists and the credit is linked. A Google Cloud project created *seconds* ago simply isn't allowed to serve yet: enabling the API and granting you owner both keep propagating for a minute or two after the commands return. Look at the error's `reason` to confirm — it says `IAM_PERMISSION_DENIED`, not `SERVICE_DISABLED`. **What to do:** nothing clever. The setup cell already retries for two minutes, which covers almost every case; if a 403 still escapes it, re-run the setup cell and give it another minute. (Running an older copy of this notebook, from before that retry existed? Re-open it from the link above.)
